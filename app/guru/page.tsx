  ...
  useEffect(() => {
    async function loadSummary() {
      ...
      try {
        ...
        const result = (await response.json()) as { data?: DashboardSummary; error?: string };
  ...
  return (
    <main className="min-h-screen px-7 py-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <header>
          <h1 className="text-4xl font-semibold text-slate-800">Dashboard Guru</h1>
          <p className="mt-1 text-xl text-slate-500">Selamat datang, Guru</p>
        </header>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-5xl font-semibold text-slate-800">
                    {isLoadingSummary ? "..." : (card as any).value}
                  </p>
                </div>
                <div className={`flex h-16 w-16 items-center justify-center rounded-2xl text-3xl ${card.iconBg}`}>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white px-8 py-7 shadow-sm">
          <h2 className="text-2xl font-semibold text-slate-800">Menu Cepat</h2>
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Link href="/guru/bank-soal" className="rounded-md border px-3 py-2">Bank Soal</Link>
            <Link href="/guru/jadwal" className="rounded-md border px-3 py-2">Jadwal Ujian</Link>
            <Link href="/guru/penilaian" className="rounded-md border px-3 py-2">Penilaian Manual</Link>
            <Link href="/guru/hasil" className="rounded-md border px-3 py-2">Hasil & Laporan</Link>
          </div>
        </section>
      </div>
    </main>
