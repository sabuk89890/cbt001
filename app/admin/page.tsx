        ...
        <section className="flex-1 overflow-y-auto px-7 py-8">
          <div className="mx-auto max-w-6xl space-y-6">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-7xl font-semibold text-slate-800">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white px-8 py-7 shadow-sm">
              ...
              {summaryMessage ? <p className="mt-3 text-xl text-red-500">{summaryMessage}</p> : null}
              {isLoadingSummary ? <p className="mt-3 text-2xl text-slate-400">Memuat data...</p> : null}
              ...
              {!isLoadingSummary && summary.latestExams.length > 0 ? (
                <ul className="mt-4 space-y-3">
                  {summary.latestExams.map((exam) => (
                    ...
                    >
                      <span className="text-xl font-medium text-slate-700">{exam.title}</span>
                      <span className="text-lg text-slate-500">{formatDate(exam.createdAt)}</span>
                    </li>
                  ))}
                </ul>
              ) : null}
